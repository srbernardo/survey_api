...
RSpec.describe 'SurveyDelete Mutation', type: :request do
  let(:survey) { create(:survey) }
  let(:query) do
    <<~GQL
      mutation($id: ID!) {
        surveyDelete(input: { id: $id }) {
          message
        }
      }
    GQL
  ...
  context 'when the input is valid and the user is authorized' do
    it 'deletes a survey and returns a message' do
      ...
      data = json['data']['surveyDelete']

      expect(response).to have_http_status(:success)
      expect(data['message']).to eq('Survey deleted successfully')
    end
  end
