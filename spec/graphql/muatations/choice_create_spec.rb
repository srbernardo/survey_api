...
RSpec.describe 'ChoiceCreate Mutation', type: :request do
  let(:question) { create(:question ) }
  let(:query) do
    <<~GQL
      mutation($value: String!, $questionId: ID!) {
        choiceCreate(input: { value: $value, questionId: $questionId }) {
          choice {
            id
            value
            questionId
  ...
  let(:variables) { { value: Faker::Book.title , questionId: question.id } }
  ...
  context 'when the input is valid and the user is authorized' do
    it 'creates a new choice and returns the choice data' do
      ...
      data = json['data']['choiceCreate']['choice']
