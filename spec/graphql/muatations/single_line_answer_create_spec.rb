...
RSpec.describe 'SingleLineAnswerCreate Mutation', type: :request do
  let(:query) do
    <<~GQL
      mutation($value: String!, $userId: ID!, $questionId: ID!) {
        singleLineAnswerCreate(input: { value: $value, userId: $userId, questionId: $questionId }) {
          singleLineAnswer {
            id
            value
            userId
  ...
  context 'when the input is valid' do
    it 'creates a new singleLineAnswer and returns the singleLineAnswer data' do
      user = create(:user )
      ...
      data = json['data']['singleLineAnswerCreate']['singleLineAnswer']
  ...
  context 'when the input is invalid' do
    it 'returns an error' do
      ...
      expect(errors.first['message']).to include("Error creating single_line_answer")
    end
  end
